...
## サンプルデータの構成(builtin 44件・Suno/Udio作り分け + ジャンル組み合わせ)
...
インストゥルメンタル(アコギ)、シティポップ、R&B。タイトル頭に `【Suno】`/`【Udio】` を付けて区別。

### ジャンル組み合わせ(フュージョン)17件 — 共通タグ `ジャンルMIX`

人気の掛け合わせを網羅。タグ `ジャンルMIX` で組み合わせ系だけを横断検索できる。
タイトルは必ず「A × B」形式(テストで検証)。追加時もこの規約を守ること。

Lo-fi×ジャズ(ジャズホップ)、シティポップ×フューチャーファンク、フォンク×トラップ(ドリフトフォンク)、
ハイパーポップ×トラップ、シンセウェイブ×ダークウェイブ、トラップ×オーケストラ、
アフロビーツ×アマピアノ、レゲトン×ネオソウル、フォークトロニカ、DnB×ジャズ(リキッド)、
K-POP×EDM×ヒップホップ、ネオソウル×ヒップホップ、シンフォニックメタル、カントリー×ポップ、
トロピカルハウス×ポップ、ネオクラシカル、ジャズ×ボサノヴァ。
これに伴い新ジャンル(ヒップホップ/エレクトロ/ラテン/アフロビーツ/フォーク/メタル/カントリー/
ジャズ/ネオソウル/K-POP)が追加され、ジャンル絞り込みが実用的になった。

### アーティスト名を入れない理由(重要・後続も厳守)

SunoもUdioも**実在アーティスト名は効かない/フィルタされる**(Udio公式「アーティストの声はコピーせず
近い音楽的質感を捉える」)。したがって「人気アーティスト風」を狙う場合も、**ジャンル×時代×楽器×
ボーカル質感×プロダクション**へ翻訳して記述するのが唯一効く方法であり、第1指示書の
「存命アーティストの模倣を推奨しない」とも一致する。データに人名を入れないこと。

出典リサーチ: Suno/Udio 各公式ヘルプ・解説ガイド(2026-07時点)。
